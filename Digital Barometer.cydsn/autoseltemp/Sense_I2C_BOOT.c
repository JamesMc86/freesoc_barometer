...
static uint8 XDATA Sense_I2C_slReadBuf[Sense_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer: Host reads responses from it */
static uint8 XDATA Sense_I2C_slWriteBuf[Sense_I2C_BTLDR_SIZEOF_WRITE_BUFFER];


/*******************************************************************************
* Function Name: Sense_I2C_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Sense_I2C_slWriteBuf - used to store received command.
*  Sense_I2C_slReadBuf - used to store response.
...
{
    /* Set Write buffer */
    Sense_I2C_SlaveInitWriteBuf(Sense_I2C_slWriteBuf, Sense_I2C_BTLDR_SIZEOF_WRITE_BUFFER);

    /* Set Read buffer which has zero elements */
    Sense_I2C_SlaveInitReadBuf(Sense_I2C_slReadBuf, 0u);
...
/*******************************************************************************
* Function Name: Sense_I2C_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Sense_I2C_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
void Sense_I2C_CyBtldrCommReset(void) CYSMALL 
{
    ...
    Sense_I2C_slStatus = 0u;
}


/*******************************************************************************
* Function Name: Sense_I2C_CyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
cystatus Sense_I2C_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
...
/*******************************************************************************
* Function Name: Sense_I2C_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from the host */
        {
            ...
            if(0u != (Sense_I2C_slStatus & Sense_I2C_SSTAT_WR_CMPLT))
            {
                /* How many bytes the host has been written */
                byteCount = Sense_I2C_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) Sense_I2C_slWriteBuf,
                              Sense_I2C_MIN_UNT16(byteCount, size));
